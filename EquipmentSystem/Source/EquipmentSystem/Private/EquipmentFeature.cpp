...
FEquipmentAbilitySetRequirement UEquipmentFeature::AbilitySetRequirementsGlobal;
...
bool FEquipmentAbilitySetRequirement::ValidateWithAbilitySets(const TArray<UExpandedAbilitySet*>& AbilitySets, TArray<FText>& OutFailedMessages) const
{
...
{
	RequiredAbilities.AddUnique(AbilityRequirement);
}

void FEquipmentAbilitySetRequirement::AddAttributeSetRequirement(TSubclassOf<UAttributeSet> AttributeSetRequirement)
{
	RequiredAttributeSets.AddUnique(AttributeSetRequirement);
}

UEquipmentFeature* FEquipmentFeatureFactory::CreateAdvancedFeature(UClass* FeatureClass)
{
	UEquipmentFeature* NewFeature = NewObject<UEquipmentFeature>(FeatureClass);
	NewFeature->PropertyModifiersOwned.Empty();
	return NewFeature;
}

FName UEquipmentFeature::GetFeatureName()
{
	FName FeatureName = StaticClass()->GetDefaultObject<UEquipmentFeature>()->FeatureName;
...
const FEquipmentPropertyModifier* UEquipmentFeature::GetPropertyModifierOwnedByFeature(FName ModifierName)
{
...
bool UEquipmentFeature::K2_GetProperty(UEquipmentFeature* Feature, FGameplayTag PropertyTag, int32& OutProperty)
{
...
{
	P_GET_OBJECT(UEquipmentFeature, OwnerFeature);
...
{
	FName PropertyName = UEquipmentSystemGlobal::GetPropertyName(PropertyTag);
	FProperty* Property = Feature->GetPropertyReflectInfo(PropertyName);
	if(!Property)
	{
...
{
	return StaticClass()->FindPropertyByName(PropertyName);
}
...
EDataValidationResult UEquipmentFeature::IsDataValid(TArray<FText>& ValidationErrors)
{
	...
	if(!AbilitySetRequirements.ValidateWithAbilitySets(AbilitySets, FailedMessages))
	{
...
void UEquipmentFeature::PostCDOContruct()
{
	...
	AbilitySetRequirements = AbilitySetRequirementsGlobal;
}

#if WITH_EDITOR
void UEquipmentFeature::AddAbilityRequirement(TSubclassOf<UEquipmentGameplayAbility> AbilityRequirement)
{
	StaticClass()->GetDefaultObject<UEquipmentFeature>()->AbilitySetRequirementsGlobal.AddAbilityRequirement(AbilityRequirement);
}

void UEquipmentFeature::AddAttributeSetRequirement(TSubclassOf<UAttributeSet> AttributeSetRequirement)
{
	StaticClass()->GetDefaultObject<UEquipmentFeature>()->AbilitySetRequirementsGlobal.AddAttributeSetRequirement(AttributeSetRequirement);
}
#else
void UEquipmentFeature::AddAbilityRequirement(TSubclassOf<UEquipmentGameplayAbility> AbilityRequirement)
{
