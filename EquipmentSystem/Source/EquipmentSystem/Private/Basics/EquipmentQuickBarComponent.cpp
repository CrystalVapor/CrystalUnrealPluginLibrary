...
#include "Basics/EquipmentQuickBarComponent.h"

#include "Basics/EquipmentManagerComponent.h"
#include "Features/EquipmentFeature_Equippable.h"
...
void UEquipmentQuickBarComponent::OnEquipmentCreated(int32 Id)
{
	...
	{
		UEquipmentFeature_Equippable* EquippableFeature = EquipmentInstance->GetFeature<UEquipmentFeature_Equippable>(UEquipmentFeature_Equippable::StaticClass());
		if(EquippableFeature)
		{
			AddToQuickBar(Id, EquippableFeature);
		}
	}
}
...
void UEquipmentQuickBarComponent::AddToQuickBar(int32 Id, UEquipmentFeature_Equippable* EquippableFeature)
{
	const FGameplayTagContainer& ActivationGroups = EquippableFeature->ActivationGroups;
	...
	NewInfo.CanActivate.BindUObject(EquippableFeature, &UEquipmentFeature_Equippable::CanActivate);
	NewInfo.CanDeactivate.BindUObject(EquippableFeature, &UEquipmentFeature_Equippable::CanDeactivate);
}
